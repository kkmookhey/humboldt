...
export async function recordModuleCDP(m, manifest) {
  ...
  try {
    ...
    for (let i = 0; i < m.sections.length; i++) {
      ...
      await page.waitForTimeout(4500); // blade content render (heavy portal SPA)
      await paintOverlay(page, {
    ...
    // Slideshow: each still held for its narration length. Offsets are exact
    // cumulative sums, so audio (placed at offsets) is perfectly in sync.
    const offsets = {};
    let t = 0, list = "";
    for (const seg of segs) {
      offsets[seg.id] = +t.toFixed(3);
      list += `file '${path.basename(seg.file)}'\nduration ${seg.dur.toFixed(3)}\n`;
      t += seg.dur;
    }
    list += `file '${path.basename(segs[segs.length - 1].file)}'\n`;
    fs.writeFileSync(path.join(sdir, "list.txt"), list);
