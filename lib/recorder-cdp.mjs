...
// Some Entra blades intermittently render a transient error shell on a cold
// hash deep-link ("Error displaying your content", "Not found") that a full
// document reload clears. Detect that sentinel and reload once before we paint.
const BLADE_ERROR = /Error displaying your content|Failed to retrieve the blade|requested view could not|could not be found|Not found/i;
async function recoverIfError(page) {
  // Dual-account sessions intermittently bounce a blade to a "Pick an account"
  // screen when a token silently refreshes mid-record (PIM/ARM blades are the
  // worst). Re-pick the configured account so the blade resumes loading.
  const acct = process.env.CDP_ACCOUNT;
  if (acct) {
    const onPicker = await page.getByText("Pick an account", { exact: false }).count().catch(() => 0);
    if (onPicker) {
      const tile = page.getByText(acct, { exact: false }).first();
  ...
  const txt = await page.evaluate(() => document.body.innerText.slice(0, 5000)).catch(() => "");
  if (!BLADE_ERROR.test(txt)) return;
...
// The portal's intermittent NPS survey ("How likely are you to recommend
// portal.azure.com…") is a fixed-position flyout that reappears across blades
// and isn't one of the labeled coach-marks above, so it leaks into frames.
// Hide its flyout by locating the heading text and climbing to the nearest
// position:fixed ancestor (the flyout itself — page content is never fixed, so
// this can't blank the blade). Best-effort.
async function dismissFeedbackSurvey(page) {
  await page.evaluate(() => {
    const needle = "How likely are you to recommend";
...
async function runDrill(page, s, total) {
  ...
  try {
    if (d.expandText) {
      // Expand-in-place drill: click a known label to reveal nested rows
      // (e.g. an MCSB control family expanding to its controls). No row-nav.
      // Then scroll that label to the top so the revealed rows fill the frame
      // (they're often far below the fold under summary cards).
      const label = page.getByText(d.expandText, { exact: false }).first();
      await label.click({ timeout: 6000 });
      await page.waitForTimeout(2000);
      // Force the expanded label to the TOP of its scroll container (not merely
      // "into view" — it's usually already visible at the bottom edge), so the
      // newly revealed child rows fill the frame below it.
      await label.evaluate((el) => el.scrollIntoView({ block: "start", behavior: "instant" })).catch(() => {});
...
export async function recordModuleCDP(m, manifest) {
  ...
  try {
    ...
    if (reuseTab) {
      page = ctx.pages().find((p) => /portal\.azure\.com|entra\.microsoft\.com/.test(p.url())) || ctx.pages()[0];
      if (!page) throw new Error("CDP_REUSE_TAB=1 but no existing portal tab found — open portal.azure.com or entra.microsoft.com in the debug Chrome and sign in first.");
    } else {
    ...
    await page.waitForTimeout(2500);
    // Dual-account sessions can land on a "Pick an account" screen instead of
    // the portal. If CDP_ACCOUNT names the account, click it to re-auth.
    if (!prof.loggedIn(page.url()) && process.env.CDP_ACCOUNT) {
      const tile = page.getByText(process.env.CDP_ACCOUNT, { exact: false }).first();
      if (await tile.count()) {
        await tile.click({ timeout: 5000 }).catch(() => {});
    ...
    if (m.sections[0]?.url) {
      ...
      await waitStable(page);
      await recoverIfError(page);
      await dismissCoachmarks(page);
      await dismissFeedbackSurvey(page);
    }
    ...
    for (let i = 0; i < m.sections.length; i++) {
      ...
      await recoverIfError(page); // reload once if the blade came up as an error shell
      await dismissCoachmarks(page); // clear feature tours before they hit a frame
      await dismissFeedbackSurvey(page); // hide the recurring NPS survey flyout
      if (s.waitMs) await page.waitForTimeout(s.waitMs); // blades that fetch list data late (e.g. PIM role settings) settle past waitStable
