...
// One-time: build assets/intro<suffix>.mp4 + assets/outro<suffix>.mp4. `bgVideo`
// (optional) is a Higgsfield clip; without it, the on-brand gradient is used.
// `introTagline` is the course label on the intro card; `suffix` lets each
// series have its own stings (e.g. "-azure") that brandModule picks per cloud.
export async function makeStings({ bgVideo, introTagline = "AWS Security Course", suffix = "" } = {}) {
  const dir = assetsDir();
  fs.mkdirSync(dir, { recursive: true });
  const brand = "NETWORK INTELLIGENCE";
  const url = "Visit us at www.networkintelligence.ai";
  const introPng = path.join(dir, `text-intro${suffix}.png`);
  const outroPng = path.join(dir, `text-outro${suffix}.png`);
  await renderTextPng({ brand, url, tagline: introTagline }, introPng);
  await renderTextPng({ brand, url, tagline: "Thanks for watching" }, outroPng);
  let bg = path.join(dir, "bg.png");
  if (bgVideo) bg = bgVideo;
  else await renderBgPng(bg);
  const intro = path.join(dir, `intro${suffix}.mp4`);
  const outro = path.join(dir, `outro${suffix}.mp4`);
  buildSting({ bg, textPng: introPng, out: intro });
  buildSting({ bg, textPng: outroPng, out: outro });
  return { intro, outro };
}
...
export function brandModule(m) {
  ...
  // Prefer per-cloud stings (e.g. intro-azure.mp4) so each series carries its
  // own course label; fall back to the default AWS stings.
  const pick = (base) => {
