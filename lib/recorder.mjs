...
import { statePath, profileDir, authPresent, BROWSER_ARGS } from "./auth.mjs";
...
export async function recordModule(m, manifest) {
  ...
  // CDP clouds (Azure/Entra) record by attaching to the user's logged-in Chrome.
  if (prof.cdp) {
    const { recordModuleCDP } = await import("./recorder-cdp.mjs");
    return recordModuleCDP(m, manifest);
  }
  if (!authPresent(m.cloud)) throw new Error(`No saved auth for ${m.cloud}. Run: node bin/login.mjs ${m.cloud}`);
  ...
  // Azure/Entra record through a persistent profile (auth spans multiple
  // domains + sessionStorage); AWS replays a saved storageState snapshot.
  let browser = null, ctx = null;
  try {
    if (prof.persistent) {
      ctx = await chromium.launchPersistentContext(profileDir(m.cloud), {
        headless: false, viewport: VIEWPORT, recordVideo: { dir: vdir, size: VIEWPORT }, args: BROWSER_ARGS,
      });
    } else {
      browser = await chromium.launch({ headless: false, args: BROWSER_ARGS });
      ctx = await browser.newContext({ viewport: VIEWPORT, storageState: statePath(m.cloud), recordVideo: { dir: vdir, size: VIEWPORT } });
    }
