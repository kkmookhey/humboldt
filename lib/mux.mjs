...
export function buildMixFilter(clips) {
  ...
  const mix = clips.map((_, i) => `[a${i}]`).join("") + `amix=inputs=${clips.length}:normalize=0:dropout_transition=0[mix]`;
  // Normalize loudness to a consistent target so different TTS voices (e.g.
  // alloy for AWS, sage for Azure) land at the same level across the library.
  const norm = `[mix]loudnorm=I=-16:TP=-1.5:LRA=11[aout]`;
  return `${delays};${mix};${norm}`;
}
